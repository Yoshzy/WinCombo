...
namespace WinCombo
{
    ...
    {
        Save sv = new Save();
        private Servicos _servicos;
        public AddS(Servicos servicos)
        {
            InitializeComponent();
            _servicos = servicos;
        }
        ...
        private void CreateF(int month)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand($"INSERT INTO `servicos`(`nome`, `usuario`, `valor`, `data`, `info`, `ano`) VALUES (@nome, @user, @valor, @vencimento, @info, {DateTime.Now.Year})", db.GetMySqlConnection());
                ...
                if (!CheckBoxes())
                {
                    if (!Fatorador())
                    {
                        if (checkName(month))
                        {
                            MessageBox.Show("Já existe um serviço com esse nome neste mês, escolha outro.", "Serviço Duplicada", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                        }
                        else
                        {
        ...
        private bool checkName(int month)
        {
            ...
            MySqlCommand cmd = new MySqlCommand($"SELECT * FROM `servicos` WHERE `nome` = @nome AND `usuario` = '{sv.User}' AND `data` = '{month}-{DateTime.Now.Year}'", db.GetMySqlConnection());
